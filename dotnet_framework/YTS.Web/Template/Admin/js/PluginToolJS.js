...
(function() {
    ...
    window.CheckData = {
        ...
        IsSizeEmpty: function(array) {
            try {
                return this.IsObjectNull(array) || array.length <= 0;
            } catch(err) {
                return true;
            }
        },
    ...
    window.PageInfo = {
        ...
        NegateClassName: function(jqElementobj, classname) {
            jqElementobj = $(jqElementobj);
            var ishas = jqElementobj.hasClass(classname);
            if (ishas) {
                jqElementobj.removeClass(classname);
    ...
    window.ConvertTool = {
        ...
        StringToArray: function(strsource, symbol) {
            return strsource.toString().split(symbol);
        },
        ToInt: function(objvalue, defint) {
            if (window.CheckData.IsObjectNull(objvalue)) {
                objvalue = "";
            }
            if (arguments.length <= 1) {
                defint = 0;

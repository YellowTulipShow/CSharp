...
namespace YTS.BLL
{
    ...
    public partial class manager
    {
        ...
        public string UserName(string user_name) {
            Model.manager model = GetModel(user_name);
        ...
        public Model.manager[] ToArray(DataSet ds) {
            if (CheckData.IsSizeEmpty(ds)) {
                return new Model.manager[] { };
            }
            List<Model.manager> list = new List<Model.manager>();
            foreach (DataTable dt in ds.Tables) {
                foreach (DataRow dr in dt.Rows) {
                    Model.manager model = this.dal.DataRowToModel(dr);
                    if (CheckData.IsObjectNull(model)) {
                        continue;
                    }
                    list.Add(model);

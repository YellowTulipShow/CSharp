...
namespace YTS.Web.tools
{
    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void user_login(HttpContext context)
        {
            ...
            {
                Utils.WriteCookie(DTKeys.COOKIE_USER_NAME_REMEMBER, "YTS", model.user_name, 43200);
                Utils.WriteCookie(DTKeys.COOKIE_USER_PWD_REMEMBER, "YTS", model.password, 43200);
            }
            else
            {
                //防止Session提前过期
                Utils.WriteCookie(DTKeys.COOKIE_USER_NAME_REMEMBER, "YTS", model.user_name);
                Utils.WriteCookie(DTKeys.COOKIE_USER_PWD_REMEMBER, "YTS", model.password);
            }

...
namespace YTS.Web.admin.article
{
    public partial class article_list : Web.UI.ManagePage
    {
        ...
        public BLL.manager bllmanager = null;

        protected void Page_Load(object sender, EventArgs e)
        {
            this.bllmanager = new BLL.manager();
            ...
            this.IsCmForumInputPeople = GetIsCmForumInputPeople(channel_id);
        ...
        protected string CombSqlTxt(string _keywords, string _property)
        {
            ...
            if (IsCmForumInputPeople) {
                strTemp.AppendFormat(" and user_name = '{0}'", GetAdminInfo().user_name);
            }
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("article_page_size", "YTSPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("article_page_size", "YTSPage", _pagesize.ToString(), 43200);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("article_list.aspx", "channel_id={0}&category_id={1}&keywords={2}&property={3}",
        ...
        /// 管理员名称
        /// </summary>
        public string ManagerName(string user_name) {
            Model.manager model = this.bllmanager.GetModel(user_name);
            if (CheckData.IsObjectNull(model)) {
                return @"错误持有人";
            }
            return string.Format("{0}({1})", model.real_name, model.user_name);
        }

        /// <summary>
        /// 论坛 URL 地址
        /// </summary>
        public string ForumURLAddress() {
            if (this.channel_name != @"cmforum") {
                return string.Empty;
            }
            StringBuilder str = new StringBuilder();
            str.Append("<div class=\"pagelist\">");
            str.Append("<div class=\"l-btns\">");
            str.Append("<span>点击查看: <a target=\"_blank\" href=\"/cmforum.html\">创美贴吧预览地址</a></span>");
            str.Append("</div>");
            str.Append("</div>");
            return str.ToString();
        }

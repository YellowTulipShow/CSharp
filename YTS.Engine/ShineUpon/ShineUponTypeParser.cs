...
namespace YTS.Engine.ShineUpon
{
    ...
    public class ShineUponTypeParser<I> where I : ShineUponInfo
    {
        private Dictionary<string, I> info_dic = null;
        private I[] info_sortAfter = null;

        public ShineUponTypeParser(Type type) {
            Analytical(type);
        }

        #region === Info ===
        public void Analytical(Type type) {
            if (CheckData.IsObjectNull(type)) {
                return;
            }
            ShineUponModelAttribute model_attr = ReflexHelp.AttributeFindOnly<ShineUponModelAttribute>(type, true);
            if (CheckData.IsObjectNull(model_attr)) {
                return;
            }

            Dictionary<string, I> dic = new Dictionary<string, I>();
            PropertyInfo[] protertys = type.GetProperties();
        ...
        public I[] GetSortResult() {
            ...
            List<I> list = new List<I>(this.GetAnalyticalResult().Values);
            if (IsNeedSort()) {
        ...
        public KeyObject GetValue_KeyObject(I info, AbsShineUpon model) {
        ...
        /// <param name="model">数据来源</param>
        /// <returns>键值数据</returns>
        public KeyString GetValue_KeyString(I info, AbsShineUpon model) {
        ...
        public AbsShineUpon SetValue_Object(I info, AbsShineUpon target_model, object ov) {

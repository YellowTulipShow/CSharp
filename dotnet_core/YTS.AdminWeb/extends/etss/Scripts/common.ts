...
function SetApiAjaxOption(option) {
    ...
    if (headers) {
        ...
        var urlData = Enumerable.from(parseQuery(url.substring(url.indexOf('?') + 1))).orderBy(a => a.key.toLowerCase()).toArray();
        var sortData = method == 'post' ? urlData : Enumerable.from($.merge(urlData, parseObjectToQueryList(option.data))).orderBy(a => a.key.toLowerCase()).toArray();
...
(jQuery as any)._ajax = jQuery.ajax;
jQuery.ajax = function (option) {
    SetApiAjaxOption(option);
    return (jQuery as any)._ajax(option);
};

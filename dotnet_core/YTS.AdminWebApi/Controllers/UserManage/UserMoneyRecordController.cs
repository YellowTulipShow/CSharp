...
namespace YTS.AdminWebApi.Controllers
{
    ...
    public class UserMoneyRecordController : BaseApiController
    {
        ...
        [HttpGet]
        public Result<object> GetUserMoneyRecord(int? ID)
        {
            if ((ID ?? 0) <= 0)
            {
            ...
            var model = db.UserMoneyRecord.Where(m => m.ID == ID).FirstOrDefault();
            return new Result<object>()
            {
                Code = ResultCode.OK,
                Data = model,
        ...
        [HttpPost]
        public Result<object> EditUserMoneyRecord(UserMoneyRecord model)
        {
            ...
            if (ID <= 0)
            {
                ...
                db.UserMoneyRecord.Add(model);
            }
            else
            {
                db.UserMoneyRecord.Attach(model);
                EntityEntry<UserMoneyRecord> entry = db.Entry(model);
        ...
        [HttpPost]
        public Result DeleteUserMoneyRecord(int[] IDs)
        {
            ...
            db.UserMoneyRecord.RemoveRange(db.UserMoneyRecord.Where(a => IDs.Contains(a.ID)).ToList());
            db.SaveChanges();

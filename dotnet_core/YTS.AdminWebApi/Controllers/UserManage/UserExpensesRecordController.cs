...
namespace YTS.AdminWebApi.Controllers
{
    public class UserExpensesRecordController : BaseApiController
    {
        ...
        [HttpGet]
        public Result<object> GetUserExpensesRecord(int? ID)
        {
            if ((ID ?? 0) <= 0)
            {
            ...
            var model = db.UserExpensesRecord.Where(m => m.ID == ID).FirstOrDefault();
            return new Result<object>()
            {
                Code = ResultCode.OK,
                Data = model,
        ...
        [HttpPost]
        public Result<object> EditUserExpensesRecord(UserExpensesRecord model)
        {
            ...
            {
                db.UserExpensesRecord.Attach(model);
                EntityEntry<UserExpensesRecord> entry = db.Entry(model);
        ...
        [HttpPost]
        public Result DeleteUserExpensesRecord(int[] IDs)
        {
            ...
            db.UserExpensesRecord.RemoveRange(db.UserExpensesRecord.Where(a => IDs.Contains(a.ID)).ToList());
            db.SaveChanges();

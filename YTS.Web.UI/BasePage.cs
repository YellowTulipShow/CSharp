...
namespace YTS.Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        ...
        /// <param name="category_id">栏目id</param>
        /// <param name="article_id">文章id</param>
        /// <returns></returns>
        protected string[] get_thumbnail_banner_img_url(int site_id = 0, string channel_name = "", int channel_id = 0, int category_id = 0, int article_id = 0) {
            const int img_size = 6;
            const string thumbnail_banner_name = @"thumbnail_banner_img";
            BLL.dv_SiteChannelFieldInfo BLL_dvSCF = new BLL.dv_SiteChannelFieldInfo();
            Model.dv_SiteChannelFieldInfo[] channels = channels = BLL_dvSCF.GetFields(thumbnail_banner_name, site_id: site_id, channel_name: channel_name, channel_id: channel_id);
            if (CheckData.IsSizeEmpty(channels)) {
                return new string[] { };
            }
            ...
            wheres.Add(@"thumbnail_banner_img != ''");
            wheres.Add(@"is_red = 1");
            if (category_id > 0) {
            ...
            foreach (Model.dv_SiteChannelFieldInfo dvSCF in channels) {
                DataSet ds = BLL_article.GetList(dvSCF.Channel_Name, img_size, article_wheres, order_str);
                articles_list.Add(BLL_article.GetModelList(ds));
            }
            while (urls.Count < img_size) {
                int lun = 0;
                int max_list_size = 0;
                foreach (Model.article[] list in articles_list) {
                    if (list.Length > max_list_size) {
                        max_list_size = list.Length;
                    }
                    if (urls.Count >= img_size) {
                        break;
                    }
                    if (lun >= list.Length) {
                        continue;
        ...
        private Model.site_channel Get_site_channel_info(string channel_name, int channel_id) {
            if (!CheckData.IsStringNull(channel_name)) {
                return new BLL.site_channel().GetModel(channel_name);
            }
            if (channel_id > 0) {
                return new BLL.site_channel().GetModel(channel_id);
            }
            return null;
        }
        ...
        public bool IsUserLogin()
        {
            ...
            {
                //检查Cookies
                string username = Utils.GetCookie(DTKeys.COOKIE_USER_NAME_REMEMBER, "YTS");
                string password = Utils.GetCookie(DTKeys.COOKIE_USER_PWD_REMEMBER, "YTS");
